			...
			when: {
				first: function(e, page) {
					...
					if($(window).width()>=2560){
						 $('#fb5-container-book').css('margin-left',"-16.7%");
					}else if($(window).width()==1920&&$(window).height()==1440){
						 $('#fb5-container-book').css('margin-left',"-17.7%");
					}else{
						 $('#fb5-container-book').css('margin-left',"-17.7%");
					}
				...
				turned: function(e, page) {
					...
					if(page_landed==""){
					if(screen.width==1920&&screen.height==1440&&page==1){
						 $('#fb5-container-book').css('margin-left',"-17.7%");
						}
					 insert_page_view(page,'double',6);
					}
					//insert_page_view(page,'double',24);
					console.log('turning');
					$('.amgen-logo').attr('style','width:200px;');
					if (page > 1) {
						$('.fb5-nav-arrow.prev').fadeIn();
                        $('#fb5-container-book').css('margin-left',0);

						//$('#fb5-about').hide();
					}

					if( (page==1&&RTL=='false') || ( page==$(this).turn('pages')&&RTL=='true') ){
						$('#fb5-about').css('z-index',12);
					...
					if (page== 10) {
						if($(window).width()>=2560){
							...
							$('#fb5-container-book').css('margin-left',"23.7%");
						}
						 ...
						 $("#fb5-book").addClass("fb5-book-bg");
						}else{
							 $("#fb5-book").removeClass("fb5-book-bg");
						}
...
set_cursor()	;
				...
				turning: function(e, page) {
					...
					play_sound();
						if(page!=1){
							if(page%2!=0){
								//alert(page-1);
								var page_pass=page-1;
							}
						}
						insert_page_view(page_pass,'double',6);
				...
				last: function(e, page) {
					$('.fb5-nav-arrow.next').hide();
					                        $('#fb5-container-book').css('margin-left',250);
	...
	arrows: function() {
		$('.fb5-nav-arrow.prev, .previous-side').click(function() {
			//$('#fb5-book').turn('previous');
            Book_v5.prevPage();
            Book_v5.resize_page()
		});
		$('.fb5-nav-arrow.next, .next-side').click(function() {
			//$('#fb5-book').turn('next');
            Book_v5.nextPage();
            Book_v5.resize_page()
		});
   ...
   setSize:function(w_,h_){
		var win_height=$(window).height();
		var win_width=$(window).width();
			// h_=  calc_height(win_height);
			  // if(win_height>900){
				// h_=890;
				// w_=1280;
			  // }

			   if((win_height>=650 && win_height<=700)&&win_width==1024){
				// h_=627;
				// w_=900;
				// h_=640;
				// w_=910;
			  ...
			  if((win_height>=650 && win_height<=700)&&(win_width==1366 || win_width==1360)){
				// h_=610;
				// w_=866;
			  ...
			  if((win_height>=650 && win_height<=700)&&(win_width==1280 || win_width==1152)){
			  ...
			  if((win_height>=790 && win_height<=949)&& win_width==1920){
 				// h_=730;
				// w_=1035;
				// h_=760;
				// w_=1079;
			  ...
			  if((win_height>=950 && win_height<=1200)&& win_width==1920){
				// h_=890;
				// w_=1265;
				// h_=920;
				// w_=1305;
			...
			if((win_height>=1201 && win_height<=1440)&& win_width==1920){
				// h_=890;
				// w_=1265;
				// h_=920;
				// w_=1305;
			 ...
			 if((win_height>=810 && win_height<=1050)&& (win_width==1680 || win_width==1600)){
				// h_=690;
				// w_=980;
				// h_=780;
				// w_=1108;
			  }
			  if((win_height>=760 && win_height<=1050)&& win_width==1440){
 				// h_=571;
				// w_=810;
				// h_=712;
				// w_=1010;
			  }
			  if((win_height>=760 && win_height<=1050)&& win_width==1400){ //not working
 				// h_=660;
				// w_=940;
			  }
	...
	//	 var posY=$('.fb5-page-book').height()/2 - $('#fb5 #fb5-cover img').innerHeight()/2;
	//	 $('#fb5 #fb5-cover img').css('top',posY+'px');
    ...
    $('.fb5-fullscreen').on('click', function() {
     ...
     $('#fb5').fullScreen({
		...
		$('.fb5-zoom-out').on('click', function() {
				...
				// All Pages
		$('.fb5-show-all, .all-pages').on('click', function() {
				if($('#fb5-all-pages').is(":visible")){
					$('#fb5-all-pages').
				removeClass('active').
				css('opacity', 0).
...
function play_sound(){
	...
	var bflat = new Audio();
	bflat.src = "https://staging.treatmentperspectives.com/flippingbook/test/sound/mp3/turnPage.mp4";
	bflat.play();
}
